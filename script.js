...
// Fixed SVGs - Stripped classes and hardcoded colors
const SVG_ICONS = {
...
function setupDataConnection(conn) {
    ...
    conn.on('data', (data) => {
        if (data.type === 'stop-screen') {
            let el = document.getElementById('cont-' + conn.peer + '-screen');
            if(el) el.remove();
...
function setupMediaCall(call) {
    ...
    let contId = 'cont-' + call.peer + '-' + type;
    call.on('stream', (stream) => {
        if (!document.getElementById(contId)) {
            ...
            v.srcObject = stream; v.autoplay = true; v.setAttribute('playsinline', 'true');
            cont.appendChild(v);
...
window.toggleAudio = () => {
    ...
    t.enabled = !t.enabled;
    document.getElementById('mute-icon-container').innerHTML = t.enabled ? SVG_ICONS.unmuted : SVG_ICONS.muted;
};

window.toggleVideo = () => {
    let t = localStream.getVideoTracks()[0];
    t.enabled = !t.enabled;
    document.getElementById('video-icon-container').innerHTML = t.enabled ? SVG_ICONS.camOn : SVG_ICONS.camOff;
};

window.shareScreen = async () => {
    if (isSomeoneSharing) return alert("Someone else is sharing.");
    try {
...
window.copyRoomCode = () => {
    ...
    navigator.clipboard.writeText(code).then(() => {
        let container = document.getElementById('copy-icon-container');
        container.innerHTML = SVG_ICONS.copyDone;
        setTimeout(() => { container.innerHTML = SVG_ICONS.copyIdle; }, 2000);
    });
};
