...
function initializePeer(id, r) {
    ...
    myPeer.on('open', (myId) => {
        ...
        if (r) {
            let conn = myPeer.connect(r, { metadata: { nick: myNick } });
            setupData(conn);
            setupCall(myPeer.call(r, localStream, { metadata: { nick: myNick } }));
        }
    });
...
function setupCall(call) {
    ...
    let nick = call.metadata ? call.metadata.nick : 'Remote';
    let cid = 'cont-' + call.peer + '-' + type;

    call.on('stream', (s) => {
        if (!document.getElementById(cid)) {
            let div = document.createElement('div');
            div.id = cid; div.className = 'video-container' + (type==='screen'?' remote-screen-share':'');
            div.innerHTML = `<div class="name-tag">${nick}${type==='screen'?' (Screen)':''}</div><button class="pin-btn" onclick="togglePin('${cid}')">📌</button><video autoplay playsinline></video>`;
...
function cleanup(peerId) {
    document.querySelectorAll(`[id*="${peerId}"]`).forEach(el => el.remove());
    // Auto-unpin if the grid is empty or pinned user left
...
window.togglePin = (id) => {
    ...
    let was = el.classList.contains('pinned');
    ...
    videoGrid.classList.remove('pinned-active');
    if(!was) {
        el.classList.add('pinned'); videoGrid.classList.add('pinned-active');
        let nr = document.createElement('div'); nr.className = 'others-row';
